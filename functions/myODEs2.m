...
% Matrices for each infection state whose rows represent age-ethnicity combination and
...
% Instantaneous IFR(t) and IHR(t) by age and susceptibility class
[antiviralsTreatedPropNow] = getAntiviralsTreatedPerUnitTime(t, par.antiviralData);
IFRt =  (1-VEf)./(1-VEi) .* par.IFR .* (1 - antiviralsTreatedPropNow .* par.antiviralsEffectIFRmult);
IHRt =  (1-VEh)./(1-VEi) .* par.IHR .* (1 - antiviralsTreatedPropNow .* par.antiviralsEffectIHRmult);

% To model deaths properly, two things need to happen
% (1) A fraction IFRt of the flow from I and A to R needs to be removed
% (2) dNdt needs to have a negative term corresponding to the removal rate,
% aggregated over susceptibility levels (columns)
removalRate = 1/par.tI * sum(IFRt .* (I+A), 2);

% ODEs for transmission & population dynamics
dNdt = calcPopnDynamics(N, par.popnBirthRate', par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities) - removalRate;
% dSdt = -(FOI*(1-VEi)) .* S + dSdt_wane + dSdt_vax               + calcPopnDynamics(S, [par.popnBirthRate, zeros(1, par.nSusComp-1)], par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
dSdt = -(FOI*(1-VEi)) .* S + dSdt_wane + dSdt_vax               + calcPopnDynamics(S, [par.popnBirthRate' zeros(numel(par.popnBirthRate), par.nSusComp-1)], par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities); % inserting zeros for each ethnicity
dEdt =  (FOI*(1-VEi)) .* S                      - 1/par.tE * E  + calcPopnDynamics(E, zeros(par.nEthnicities, par.nSusComp), par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
dIdt = 1/par.tE * (    par.pClin.*(1-VEs)./(1-VEi)) .* E - 1/par.tI * I  + calcPopnDynamics(I, zeros(par.nEthnicities, par.nSusComp), par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
dAdt = 1/par.tE * (1 - par.pClin.*(1-VEs)./(1-VEi)) .* E - 1/par.tI * A  + calcPopnDynamics(A, zeros(par.nEthnicities, par.nSusComp), par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
dRdt = 1/par.tI * (1-IFRt) .* (I+A) - waneRate_RtoS_dynamic * R                     + calcPopnDynamics(R, zeros(par.nEthnicities, par.nSusComp), par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
dVdt = [nDose1, nDose2, nDose3]                                     + calcPopnDynamics(V, zeros(par.nEthnicities, 3), par.popnDeathRate, par.popnAgeingRate, par.nAgeGroups, par.nEthnicities);
...
% Get current testing probabilities (at time t) by interpolating the vector
% of daily testing probabilities
pTestClinNow = interp1(par.tBase', par.pTestClin', t)';
pTestSubNow = interp1(par.tBase', par.pTestSub', t')';
...
newHospToBe_all = 1/par.tE * IHRt .* E;
...
newDeathsToBe_all = 1/par.tE * IFRt .* E ;
...
% Restructure as column vector, recalling that the last column of R is not
% included as a state variable as it is recovered from the other variables
% and the known pop size in each age-ethnicity combination
